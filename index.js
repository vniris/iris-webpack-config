...
module.exports = function(options = {}) {
  return {
    ...
    externals: [
      {
        '@iris/core/forum': 'iris.core',
        '@iris/core/admin': 'iris.core',
        'jquery': 'jQuery',
      },
          ...
          for (const extension of options.useExtensions) {
            externals['@'+extension] =
              externals['@'+extension+'/forum'] =
                externals['@'+extension+'/admin'] = "iris.extensions['"+extension+"']";
          }
        }
      ...
      function(context, request, callback) {
        let matches;
        if ((matches = /^iris\/(.+)$/.exec(request))) {
          return callback(null, 'root iris.core.compat[\'' + matches[1] + '\']');
        }
        callback();
      }
    ],
